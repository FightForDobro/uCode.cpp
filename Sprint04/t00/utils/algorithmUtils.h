...
namespace Utils
{
    // [from, to]
    template<typename T>
    bool IsInRange(const T &val, const T &from, const T &to) { return (val - from) * (val - to) <= 0; }
    ...
    // (from, to)
    template<typename T>
    bool IsInsideRange(const T &val, const T &from, const T &to) { return (val - from) * (val - to) < 0;}
    ...
    void RemoveAll(Collection &c, const T &value) { c.erase(std::remove(c.begin(), c.end(), value)); }
    ...
    template<class Collection, class T>
    auto Find(Collection &c, const T &value) { return std::find(c.begin(), c.end(), value); }
    ...
    template<class Collection, class T>
    bool Contains(const Collection &c, const T &value) { return std::find(begin(c), end(c), value) != c.end(); }
    ...
    auto MaxElement(const Collection &c) { return std::max_element(c.begin(), c.end()); }

    template<class Collection>
    auto MinElement(const Collection &c) { return std::min_element(c.begin(), c.end()); }

    template<class Collection, class Comp>
    auto MaxElement(const Collection &c, Comp &&comparator) { return std::max_element(c.begin(), c.end(), comparator); }
    ...
    void Sort(Collection &c)
    {
        for (auto it = c.begin(); it != c.end(); ++it)
            std::rotate(std::upper_bound(c.begin(), it, *it), it, std::next(it));
    }
    ...
    {
        for (auto it = c.begin(); it != c.end(); ++it)
            std::rotate(std::upper_bound(c.begin(), it, *it, std::forward<Comp>(comparator)), it, std::next(it));
    }
    ...
    {
        return std::distance(begin(c), std::find_if(begin(c), end(c), std::forward<Pred>(predicate)));
    }
}
